 ...
 *
 * Flow:
 *   1. Fetch a batch of randomized exercises from the backend
 *   2. Render one exercise at a time
 *   3. Submit each answer, show immediate feedback
 *   4. Track how many unique exercises were answered correctly
 *   5. Once practice_minimum is met, show completion screen
 */
...
export default function PracticePage() {
  ...
  const handleAnswerResult = (isCorrect: boolean) => {
    ...
    // Advance to next exercise after a short delay (handled inside ExerciseCard)
  };
...
function ExerciseCard({ exercise, onResult, onNext }: ExerciseCardProps) {
  ...
  useEffect(() => {
    // Auto-focus input on new exercise
  ...
  <div className="mt-4 p-4 bg-red-100 text-red-800 text-xl font-bold">
    DEBUG: {exercise.number_string ?? "NO NUMBER STRING"}
  </div>
)}
          ...
          <div>
            ...
            {!result.is_correct && result.correct_answer && (
              <p className="text-sm mt-1 opacity-80">
                Răspuns corect: <span className="font-mono font-semibold">
                  <InlineMath text={result.correct_answer} />
                </span>
              </p>
            )}
          </div>
            ...
            <button
              ...
              className={`
                ...
                ${isCorrect
                  ...
                  : "border-gray-200 bg-white text-gray-800 hover:border-indigo-400 hover:bg-indigo-50 hover:scale-105 cursor-pointer"
                }
              `}
            >
              {digit}
